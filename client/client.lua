...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('kg-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('kg-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('kg-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('kg-weathersync:client:DisableSync', function()
    disable = true
    SetRainLevel(0.0)
...
RegisterNetEvent('kg-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('kg-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)
